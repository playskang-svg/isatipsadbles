...
Cowork 세션과 로컬(안티그래비티 · Claude Code)이 **같은 폴더를 동시에 만진다.** 별도 사본이 아니라 하나의 실물이다. 2026-09-05에 이 구조에서 락 충돌 4회, node_modules 플랫폼 충돌 1회, 남의 미커밋 변경이 커밋에 섞일 뻔한 사고 1회가 났다. 아래는 그 대응이다.

### 같은 파일을 동시에 만지지 않는다

파일을 통째로 다시 쓰는 경우가 있어 병합이 일어나지 않는다. 나중에 쓴 쪽이 상대 작업을 조용히 덮어쓴다. 작업 영역을 나눈다 — 예: 글 데이터(`lib/*-articles.ts`)는 Cowork, 컴포넌트·라우트는 로컬.

작업을 시작하기 전에 `git --no-optional-locks status`로 작업 트리에 남의 미커밋 변경이 있는지 본다. 있으면 그 파일은 건드리지 않는다.

### 커밋 전에 변경 목록을 확인한다

`ship`이 1단계에서 변경 파일을 출력한다. 내가 만들지 않은 파일이 섞여 있으면 그대로 담지 말고 별도 커밋으로 분리하거나 상대에게 확인한다. `git add -A`를 무심코 돌리면 남의 진행 중인 작업이 올라간다.

### git 쓰기는 로컬에서만 한다

Cowork 세션은 파일만 고치고 커밋하지 않는다. 읽기용 git도 반드시 `git --no-optional-locks ...`로 호출한다. 그냥 `git status`조차 인덱스를 갱신하며 `index.lock`을 만드는데, 삭제 권한이 만료된 샌드박스에서는 그 락을 회수하지 못해 로컬 커밋이 막힌다.

락이 남아 막히면 파일 시각을 먼저 본다. 방금 것이면 상대가 작업 중일 수 있으니 기다리고, 오래된 0바이트 잔여물이면 지운다. `ship`의 사전점검이 1분 기준으로 갈라서 안내한다.
...
node_modules는 맥 소유다(8장). 양쪽에서 설치하면 네이티브 바인딩이 서로를 덮어쓴다.

### 빌드 EPERM

연결 폴더에서 빌드가 `EPERM ... unlink dist/...`로 실패하면 세션에 파일 삭제 권한이 없는 것이다. 권한을 받은 뒤 `rm -rf dist`하고 다시 빌드한다. Cowork에서는 애초에 빌드하지 않는 것이 원칙이다.
